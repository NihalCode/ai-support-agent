...
async function runSchemaMigrations(sql: NeonSql): Promise<void> {
  ...
  await sql`
    ALTER TABLE app_users ADD COLUMN IF NOT EXISTS invited_by_user_id TEXT
  `;
  await sql`
    ALTER TABLE app_users ADD COLUMN IF NOT EXISTS accepted_invite_at TIMESTAMPTZ
  `;
  await sql`
    ALTER TABLE app_users ADD COLUMN IF NOT EXISTS picture TEXT
  `;
  ...
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS idx_audit_logs_org_at
  ...
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS idx_system_health_org_status
    ON system_health_events (org_id, status, created_at DESC)
  `;
  ...
  await sql`
    ...
    ON app_notifications (org_id, user_id, read, created_at DESC)
  `;
